...
const AdminAllRequestsPage = () => {
    ...
    const [loading, setLoading] = useState(true);
    const [updatingId, setUpdatingId] = useState(null);
    const [openMenu, setOpenMenu] = useState(null); // holds req._id
    ...
    // ✅ ১. সার্চের জন্য Debounce লজিক (যা সার্ভার ক্র্যাশ হওয়া থেকে বাঁচাবে)
    useEffect(() => {
        const handler = setTimeout(() => {
            setDebouncedSearch(searchTerm);
            setCurrentPage(1); // সার্চ পরিবর্তন হলে পেজ ১ এ নিয়ে যাবে
        }, 500);

        return () => clearTimeout(handler);
    }, [searchTerm]);

    // ✅ ২. ড্রপডাউন মেনু বাইরে ক্লিক করলে বন্ধ করার গ্লোবাল লজিক (উইদাউট রিজিড রিফ)
    useEffect(() => {
    ...
    // ✅ ডেটা ফেচ করার এফেক্ট
    useEffect(() => {
        if (!user) return;
        fetchRequests();
    }, [currentPage, statusFilter, debouncedSearch, user]); // debouncedSearch ব্যবহার করা হয়েছে
                                    ...
                                    return (
                                        <tr key={req._id} className="hover:bg-zinc-950/50 transition-colors">
                                            ...
                                            <td className="p-4 text-center relative" onClick={(e) => e.stopPropagation()}>
                                                <button
                                                    onClick={(e) => {
                                                        e.stopPropagation(); // বাইরে ক্লিক হ্যান্ডেলার যেন ট্রিগার না হয়
                                                        setOpenMenu(openMenu === req._id ? null : req._id);
                                                    }}
                                                    className="p-2 rounded-lg bg-zinc-800 hover:bg-zinc-700"
                                                >
                                                ...
                                                {openMenu === req._id && (
                                                    <div className="absolute right-5 top-12 z-20 w-40 bg-zinc-900 border border-zinc-700 rounded-xl shadow-xl overflow-hidden">
                                                        <Link
                                                            href={`/donation-requests/${req._id}`}
                                                            onClick={() => setOpenMenu(null)}
                                                            className="w-full flex items-center gap-2 px-4 py-3 text-sm hover:bg-zinc-800 text-zinc-200"
                                                        ...
                                                        <button
                                                            ...
                                                            className="w-full flex items-center gap-2 px-4 py-3 text-sm hover:bg-zinc-800 text-blue-400 text-left"
                                                        >
                                                            <FaEdit /> Edit
                                                        </button>
                                                        ...
                                                        <button
                                                            ...
                                                            className="w-full flex items-center gap-2 px-4 py-3 text-sm hover:bg-zinc-800 text-red-500 text-left"
                                                        >
                                                            <FaTrash /> Delete
                                                        </button>
                                                    </div>
            ...
            {editingRequest && (
                <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4">
                    <div className="bg-zinc-900 border border-zinc-700 rounded-2xl w-full max-w-md p-6">
                        <div className="flex justify-between items-center mb-6">
                            <h3 className="text-xl font-bold text-white">Edit Request</h3>
                            <button onClick={handleEditClose} className="text-zinc-400 hover:text-white">
                                <FaTimes size={18} />
                            </button>
                        </div>
