...
// ---------------------------------------------------------------------------
// Single nested location object: Division -> District -> [Upazilas]
// ---------------------------------------------------------------------------
...
const inputBase =
  'w-full pl-11 pr-4 py-3 bg-zinc-950/50 border rounded-xl text-sm focus:outline-none focus:bg-zinc-900/80 transition-all text-zinc-100 placeholder-zinc-700 font-medium';
const selectBase =
  'w-full pl-11 pr-10 py-3 bg-zinc-950/50 border rounded-xl text-sm focus:outline-none focus:bg-zinc-900/80 transition-all text-zinc-300 font-medium appearance-none cursor-pointer';
...
const CreateDonationRequestPage = () => {
  ...
  // 1. Live session data fetch করা হচ্ছে
  const { data: session } = useSession();
  const user = session?.user;

  console.log(user);

  // 2. সেশন থেকে ইউজার ডাটা পাওয়া মাত্রই ফর্ম স্টেটে সেট করে দেওয়া হচ্ছে
  useEffect(() => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // সাবমিট সফল হলে ইউজারের নাম ও ইমেইল ঠিক রেখে বাকি ফিল্ড রিসেট করা হচ্ছে
      setForm({
        ...initialForm,
        name: form.name,
        email: form.email,
      ...
      setErrors((prev) => ({ ...prev, submit: 'Could not send the request. Try again.' }));
    } finally {
      setSubmitting(false);
    }
              ...
              <Field label="Blood Group Needed" error={errors.bloodGroup}>
                <IconField icon={<FiDroplet size={16} />}>
                  ...
                  >
                    ...
                    {BLOOD_GROUPS.map((bg) => (
                      <option key={bg} value={bg} className="bg-zinc-950 text-white">
                        {bg}
                      </option>
                    ))}
                  </select>
                ...
                <IconField icon={<FiMapPin size={16} />}>
                  <select
                    ...
                    className={`${selectBase} ${
                      errors.district ? borderErr : borderOk
                    } disabled:opacity-40`}
                  >
                    <option value="" disabled className="bg-zinc-950 text-zinc-600">
              ...
              <Field label="Upazila" error={errors.upazila}>
                <IconField icon={<FiMapPin size={16} />}>
                  <select
                    ...
                    className={`${selectBase} ${
                      errors.upazila ? borderErr : borderOk
                    } disabled:opacity-40`}
                  >
                    <option value="" disabled className="bg-zinc-950 text-zinc-600">
