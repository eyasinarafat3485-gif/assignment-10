...
import {
  ...
  FiChevronDown, FiUploadCloud, FiCheckCircle, FiTrash2
} from 'react-icons/fi';
...
// বাংলাদেশের ৮টি বিভাগ এবং ৬৪টি জেলার সম্পূর্ণ ডাইনামিক ডেটা স্ট্রাকচার (ফিক্সড)
...
export default function RegisterSection() {
  ...
  // Converts the uploaded profile image File into a base64 data URL so it
  // can be passed straight into authClient.signUp.email's "image" field.
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const imageDataUrl = await fileToBase64(formData.profileImage);

      // Same authClient.signUp.email call shape used on the login/register page.
      // Extra donor fields are passed along too — make sure these are configured
      // as additionalFields on the better-auth user schema on the backend.
  ...
  return (
    <section className="w-full py-16 bg-zinc-950/70 backdrop-blur-md text-white font-sans border-t border-zinc-900 flex items-center justify-center relative overflow-hidden">
          ...
          <div className="lg:col-span-8 p-8 md:p-10 flex flex-col justify-center">
            <div className="mb-8 lg:mb-6 space-y-2">
              ...
              <p className="text-zinc-400 text-xs md:text-sm font-medium">
                Fill out the required credentials to launch your verified donor account.
              </p>
            </div>

            <form onSubmit={handleSubmit} className="space-y-5">
              
              {/* Functional Profile Upload Avatar Row (REQUIRED) */}
              <div 
                onClick={() => fileInputRef.current?.click()}
                className={`flex flex-col sm:flex-row items-center gap-4 p-4 rounded-2xl bg-zinc-950/40 border transition-all cursor-pointer group ${
                  imagePreview ? 'border-red-500/40 bg-zinc-900/30' : 'border-zinc-900/60 hover:border-zinc-700'
                }`}
              >
                <input 
                  type="file" 
                ...
                <div className="text-center sm:text-left flex-1">
                  ...
                  <p className="text-[10px] text-zinc-500 mt-0.5">
                    {imagePreview ? 'Image loaded successfully. Click the trash icon to reset.' : 'Click to browse or drop an official passport image. Max 2MB (Required).'}
                  </p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  ...
                  <div className="relative">
                    ...
                    <input 
                      type="tel" required 
                      className={`w-full pl-11 pr-4 py-3 bg-zinc-950/50 border rounded-xl text-sm focus:outline-none focus:bg-zinc-900/80 transition-all text-zinc-100 font-medium tracking-wide ${
                        phoneError ? 'border-red-500' : 'border-zinc-800 focus:border-red-500/50'
                      }`}
              ...
              {/* Dynamic Location Area (Division -> District -> Upazila) */}
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                ...
                {/* 2. District Selector */}
                <div className="space-y-1.5">
                  <label className="text-xs font-bold text-zinc-400 uppercase tracking-wider">District (All)</label>
                  <div className="relative">
                    <FiMapPin className="absolute left-4 top-1/2 -translate-y-1/2 text-zinc-500" size={16} />
                    <select 
                      required
