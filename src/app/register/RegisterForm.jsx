...
//             //     toast.success(
//             //         'Registration successful! Welcome to the lifesaving community.'
//             //     );
...
//                 </div>
//             </div>
//             {/* ২. Role Selection (Fixed to Donor / Volunteer) */}
...
export default function RegisterForm() {
    ...
    const handleImageChange = (e) => {
        const file = e.target.files[0];
        if (file) {
            // ৫MB এর জায়গায় ২MB লিমিট ঠিক করা হলো মেসেজের সাথে মিল রেখে
            if (file.size > 2 * 1024 * 1024) {
                toast.error('File size is too large! Maximum limit is 2MB.');
                return;
            }
    ...
    //         // এখানে আসল এররটি সঠিকভাবে ধরার ব্যবস্থা করা হলো
    ...
    // ১. প্রোফাইল ইমেজ আছে কি না চেক
    if (!formData.profileImage) {
        toast.error('Profile photo is required to join the lifesaving community!');
        return;
    }

    // ২. প্রথম ফাংশনের মতো ইমেজ সাইজ ৫MB এর বেশি কি না চেক (Size validation)
    ...
    try {
        ...
        // প্রথম ফাংশনের মতো ডাটা সাকসেসফুল না হলে এরর টোস্ট দেখানো
        if (!imgbbResponse.ok || !imgbbData.success) {
            const errMsg = imgbbData.error?.message || 'Failed to upload profile image.';
            toast.error(`ImgBB Error: ${errMsg}`);
            setIsSubmitting(false);
        ...
        // সফলভাবে আপলোড হলে URL নেওয়া
        const imageUrl = imgbbData.data.url;

        // আপনার AuthClient রেজিস্ট্রেশন লজিক
        const { data, error } = await authClient.signUp.email({
            email: formData.email.trim(),
            password: formData.password,
            name: formData.fullName.trim(),
        ...
        console.error(err);
        // প্রথম ফাংশনের নেটওয়ার্ক এরর ক্যাচ লজিক
